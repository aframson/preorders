    ...
    <>
      ...
      {!drops?.length ? (
        ...
        <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {drops.map((drop) => {
            // The live batch is whichever one is open; failing that, the most
            // recent, so an idle drop still shows where it got to.
            ...
            return (
              <li key={drop.id}>
                <Link
                  ...
                  className="block rounded-card border border-border bg-surface p-4 transition-colors hover:border-brand-300"
                >
